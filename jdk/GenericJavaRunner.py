...
class GenericJavaRunner(DockerBasedStandardRunner):
    ...
    def get_command(self, request: RunnerRequest):
        ...
        if self.version >= 11:
            return f"java ./target/{request.submission_id}.java"
        else:
            ...
            with open(f"./target/{request.submission_id}.java", 'rb') as src_file, \
                    open(f"./target/olderJDK/{request.submission_id}/Main.java", 'wb+') as dest_file:
                dest_file.write(src_file.read())
            # 编译，执行 Java 文件
            return f"javac ./target/olderJDK/{request.submission_id}/Main.java;cd ./target/olderJDK/{request.submission_id}/;java Main"
